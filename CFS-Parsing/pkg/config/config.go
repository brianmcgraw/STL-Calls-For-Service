...
func New() (*Config, error) {
	...
	// c.ShouldCallMaps = c.GetShouldCallMapsAPIEnv()
	dbname := c.GetDB()

	psqlInfo := fmt.Sprintf("host=%s port=%s user=%s "+
		"password=%s dbname=%s sslmode=disable",
		postgresURL, port, user, password, dbname)
...
func (c Config) GetTable() string {
	viper.BindEnv(POSTGRES_TABLE_ENV)
	if !viper.IsSet(POSTGRES_TABLE_ENV) {
		log.Fatalf("Unable to retrieve environment variable: %v", POSTGRES_TABLE_ENV)
	}
	return viper.GetString(POSTGRES_TABLE_ENV)
...
// func (c Config) GetShouldCallMapsAPIEnv() bool {
// 	viper.BindEnv(SHOULD_CALL_MAPS_ENV)
// 	if !viper.IsSet(SHOULD_CALL_MAPS_ENV) {
// 		log.Fatalf("Unable to retrieve environment variable: %v", SHOULD_CALL_MAPS_ENV)
// 	}

// 	value := viper.GetString(SHOULD_CALL_MAPS_ENV)

// 	if value == "true" {
// 		return true
// 		log.Println("Call Google Maps boolean set to true")
// 	}
// 	log.Println("Call Google Maps boolean set to false")
// 	return false
// }
